...
router.post(
  ...
  [
    ...
    body('selectedStokvel').isInt().withMessage('Please select a stokvel to join'),
    validate,
  ],
  async (req, res) => {
    try {
      ...
      // Verify stokvel exists and has capacity
      const [stokvels] = await pool.query(
        'SELECT id, name, max_members, target_amount FROM stokvels WHERE id = ?',
        [selectedStokvel]
      );
      if (stokvels.length === 0) {
        return res.status(404).json({ error: 'Selected stokvel not found' });
      }
      ...
      if (memberCount[0].count >= stokvel.max_members) {
        return res.status(400).json({ error: 'This stokvel is full' });
      }

      const passwordHash = await bcrypt.hash(password, 12);

      // Create user with 'pending' status — admin must approve
      const [result] = await pool.query(
        'INSERT INTO users (full_name, email, phone, password_hash, role, status) VALUES (?, ?, ?, ?, ?, ?)',
        [fullName, email, phone || null, passwordHash, 'member', 'pending']
      );

      // Create a join request so admin can approve stokvel membership
      await pool.query(
        'INSERT INTO join_requests (user_id, stokvel_id, status) VALUES (?, ?, ?)',
        [result.insertId, selectedStokvel, 'pending']
      );
      ...
      for (const admin of admins) {
        await pool.query(
          ...
          [admin.id, 'info', 'New Registration', `${fullName} has registered and wants to join ${stokvel.name}. Please review their application.`]
        );
      }
